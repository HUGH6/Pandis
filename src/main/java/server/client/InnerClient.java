...
public class InnerClient implements Client {
     ...
     * 创建Client的静态工厂
     * 为什么使用静态工厂：
     * client对象会持有reciever和replyer对象，而reciever和replyer创建时又会反过来持有client的引用
     * 如果直接用new的方式创建client，那么，就会在client没有初始化完全时，将this引用暴露给reciever和replyer
     * 而reciever和replyer构造函数内部对client属性的访问就会出现问题。
     * 因此需要使用静态工厂，先创建完成client，然后通过属性设置的方式填充其属性
     * @param socketChannel
     * @return
     */
    public static Client createClient(SocketChannel socketChannel) {
        ...
        client.database = ServerContext.getContext().getDatabases()[0];
    ...
    public void replyStatus(PandisString s) {
    ...
    public void replyError(PandisString s) {
