...
public class AOFPersistence {
    ...
    public void startAppendOnly() {
        ...
        try {
            ...
            logger.error("Pandis needs to enable the AOF but can't open the append only file: " + this.aofFileName, e);
        }
            ...
            if (!force) {
                if (fsyncInProgress) {
                    ...
                    if (this.aofFlushPostponedStart == 0) {
                        ...
                        // 那么执行写操作（write 将被阻塞）
                        logger.info("Asynchronous AOF fsync is taking too long (disk is busy?). Writing the AOF buffer without waiting for fsync to complete, this may slow down Pandis.");
                    }
                }
            }
        }
     ...
     * @param dbId
     */
    public void feedCommand(List<Sds> command, int dbId) {
        // 使用 SELECT 命令，显式设置数据库，确保之后的命令被设置到正确的数据库
        if (dbId != this.aofSelectedDbId) {
            this.aofSelectedDbId = dbId;

            // 生成select命令，添加到缓冲区
            String id = String.valueOf(dbId);
            Sds selectCommand = Sds.createSds("*2\r\n$6\r\nSELECT\r\n$" + id.length() + "\r\n" + id + "\r\n");
            this.aofBuffer.append(selectCommand);
        }
