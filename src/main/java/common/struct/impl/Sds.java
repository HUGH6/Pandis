 ...
 * @create: 2021-09-28
 */
public class Sds implements PandisString, Comparable<Sds> {
    /**
    ...
    @Override
    public void append(PandisString str) {
     ...
     * 返回Sds的所有字母小写的新Sds
     */
    public Sds toLowerCase() {
        String str = toString().toLowerCase();
        byte [] bytes = SafeEncoder.encode(str);
        return new Sds(bytes.length, 0, bytes);
    }
     ...
     * 将字符串中所有字符转换成大写
     */
    public Sds toUpperCase() {
        String str = toString().toUpperCase();
        byte [] bytes = SafeEncoder.encode(str);
        return new Sds(bytes.length, 0, bytes);
    }
